<?php
/**
 * RIF-II.ORG Document Upload Script
 * 
 * INSTRUCTIONS:
...
try {
    ...
    error_log("Document upload attempt from: " . $_SERVER['REMOTE_ADDR']);
    error_log("Files received: " . print_r($_FILES, true));
    
    // Check if file was uploaded (try multiple field names)
    $uploadedFile = null;
    $fieldNames = ['file', 'document', 'upload'];
    ...
    if (!in_array($mimeType, $allowedTypes)) {
        throw new Exception('Invalid file type: ' . $mimeType . '. Only PDF, DOC, and DOCX files are allowed.');
    }
    ...
    error_log("Document uploaded successfully: " . $targetPath);
